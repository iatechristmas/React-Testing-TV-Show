...
import { render, waitFor, wait, findAllByRole } from "@testing-library/react";
...
test("successfully renders data from api", async () => {
  ...
  const { findByText, findAllByRole, findAllByTestId } = render(<App />);
  const div = await findByText(/Select a season/i);

  userEvent.click(div);

  const options = await findAllByRole(/option/i);
  ...
  const episodes = await findAllByTestId(/fuck/i);
});
